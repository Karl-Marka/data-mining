...
def predict(filename, file):
    ...
    predictedScores = output.copy()
    for n,i in enumerate(output):
        if i < 0.59:           
            output[n] = 'Normal'
        else:  
            output[n] = 'Scleroderma'
    #predictedScores = predictedScores.tolist()
    output = pd.DataFrame(data = [output, predictedScores], columns = names, index = ['Type', 'Value'])
    output = output.T
    print(output)
    outputFile(output,file)

def outputFile(output,file):
    filename = str(file) + '.predictions.values.txt'
    output.to_csv(filename, sep = "\t")
    print ('Predictions saved to file ' + str(filename))
    closeFunc()
